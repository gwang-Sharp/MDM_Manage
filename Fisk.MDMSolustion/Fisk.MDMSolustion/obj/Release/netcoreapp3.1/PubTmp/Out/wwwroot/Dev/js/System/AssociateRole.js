var AssociateRole = new Vue({
    ...
    data: {
        ...
        IsTableShow: false,
        IsFenYeShow: false,
        checked: false,    //cube 是否全选
        IsDisPlaycheckbox: false, //cube复选框是否显示
        radio: '1',
        ...
        UserUserName: '',
        CubeSaveType: false,
        ModifyRoleIdList: {},     //角色集合
        ReportPermissionTreeId: '',   //选中报表权树节点的Id
        TabNumber: '',
        ReportPermissionTreeDisAble: '',
        ReportPermissionTree: [],   //报表权限树集合
        ReportPermissionTreeProps: {   // 报表权限树绑定
            children: 'children',
            label: 'label'
        },
        MenuPermissionTreeID: '',       //选中菜单树节点的Id
        MenuPermissionTree: [],     // 菜单权限树集合
        MenutPermissionTreeProps: {   // 菜单权限树绑定
            children: 'children',
            label: 'label'
        },
        Buttondisabled: true,  //提交按钮
        UploadPermissionTreeID: '',
        UploadPermissionTree: [],//  上传树集合
        UploadPermissionTreeProps: {   // 菜单权限树绑定
            children: 'children',
            label: 'label'
        },
        DistributionUserTableDataId: '',
        DistributionUserTableData: [],  //分配用户集合
        UserQueryParameters:
        {
            page: 1,
            limit: 10,
        ...
        FolderPermissionTree: [],//文件夹权限数据集合
        FolderPermissionTreeProps: {
            children: 'children',
            label: 'label'
        },
        TabularOptions: [],  //级联选择器数据
        TabularValue: [],
        CubeValue: [],
        CubeOptions: [],  //cube表格数据集合
        ...
        CubeTableData: [],//cube表格数据集合
        CubeCount: 0,   //cube表格数据长度
        EidCubeDataRolParms:
        {
            Description: '',
            MEMBERKEY: '',
        ...
        BoxLoading: false,
        AddHtmlLoading: false,  //添加用户laoding
        AddRepoerHtmlLoading: false, //添加报表loading
        AddCubeHtmlLoading: false, //添加cubeloading
        AddMenuHtmlLoading: false,//添加菜单
        AddUplodHtmlLoading: false, //上传
        AddJobHtmlLoading: false,//job
        AddRolHtmlLoading: false,//角色
        rolePageMaxSize: 0,
        screenHeight: "",
    ...
    created: function () {
        ...
        this.screenHeight = parseInt(LayOutStaticWindowsHeight - 240) + "px";
    ...
    {
        //获取用户分配数据请求
        GetUsetDataTableList: function (type) {
            var QueryParameters;
            if (type == 1) {
                QueryParameters = {
                    page: 1,
            ...
            this.UserTableTable = true;
            //console.log("loading::"+ AssociateRole.Rolloading);
            $.post('/system/UserGet', { page: QueryParameters.page, rows: QueryParameters.limit, where: QueryParameters.where}, function (res) {
        ...
        //获取菜单层级结构数据请求
        GetMenuPermissionTree: function () {
        ...
        //角色table获取表格数据
        GetListRoles: function (type, ParmsId, types) {
            debugger
            this.Rolloading = true;
            var url;
            if (types == 1) {
                ...
                if (res.success) {
                    ...
                    AssociateRole.$nextTick(() => {
                        ...
                        if (AssociateRole.CheckedBoxRolTable != null) {
                            AssociateRole.tableData.forEach(e => {
        ...
        //角色table分页改变每页条数
        TableDataSize_change: function (valSaze) {
        ...
        selectCheckBox: function (selection, row) {
            ...
            ////selected为 true就是选中，0或者false是取消选中
           
          
                AssociateRole.ReportRoleManagesTheJSON.forEach(res => {
        ...
        //点击角色table表头复选框
        selectCheckBox_ALL: function (selection) {
            IsHasPermissionsData = true;
            let selectedall = false;  ////selected为 true就是选中，false是取消选中
            selection.forEach(e => {
                selectedall = this.tableData.indexOf(e) !== -1;
            });
            ...
            if (selectedall) {
                selection.forEach(e => {
                    if (this.CheckedBoxRolTable.indexOf(e.id + '') < 0) {
                        this.CheckedBoxRolTable.push(e.id + '');
                    }
                });
            } else {
                this.tableData.forEach(e => {
                    if (this.CheckedBoxRolTable.indexOf(e.id + '') > -1) {
                        this.CheckedBoxRolTable.splice(this.CheckedBoxRolTable.indexOf(e.id + ''), 1);
                    }
                });
            }
            if (AssociateRole.tableDataCount === AssociateRole.CheckedBoxRolTable.length) {
             ...
             if (AssociateRole.TabNumber == 'Eignt') {
                if (AssociateRole.MenuPermissionTreeID != null && AssociateRole.MenuPermissionTreeID != "") {
                    ...
                    if (RolIdList != null && RolIdList[0] != null) {
                        ...
                        $.post('/System/ModifMenuRole', { FilNoList: RolIdList }, function (res) {
                            if (res.success == true) {
                                AssociateRole.Rolloading = false;
                ...
                if (AssociateRole.DistributionUserTableDataId != null && AssociateRole.DistributionUserTableDataId != "") {
                    ...
                    if (RolIdList != null && RolIdList.length > 0) {
                        AssociateRole.BoxLoading = true;
                        $.post('/System/ModifUserRole', { FilNoList: RolIdList }, function (res) {
                            if (res.success == true) {
                                AssociateRole.Rolloading = false;
                                // if (res.otherMsg!=null && res.otherMsg.success == true) {
                                AssociateRole.$message({
                                    type: 'success',
                                    message: res.message
                                });
                                ...
                                //}
                                    ...
                                    AssociateRole.ReportRoleManagesTheJSON = [];
                                    AssociateRole.RoleQueryParameters.page = 1;
                                    AssociateRole.GetListRoles(0, AssociateRole.ReportPermissionTreeId, 0);
        ...
        //点击菜单获取角色被选中
        MenuPermissionTreeNodeClick: function (even, node, de) {
            if (node.data != null) {
                AssociateRole.MenuPermissionTreeID = node.data.id;
                //if (AssociateRole.MenuPermissionTreeID == 1) {
                //    AssociateRole.Buttondisabled = true;
                ...
                if (type == 0) {
                    ...
                    $.post('/System/GetMenuNodeCheckRoleList', { MenuId: node.data.id }, function (res) {
        ...
        //用户分页改变页码
        UserhandleCurrentChange_row: function (val) {
        ...
        //用户分页改变条数
        UserTableDataSize_changesize: function (valSaze) {
        ...
        //获取用户对应的角色权限
        UserTablehandleCurrentChange: function (row, column, event) {
            ...
            if (row != null) {
                ...
                if (type == 0) {
                    $.post('/System/GetUserAllCheckedRole', { UserID: row.id }, function (res) {
                        if (res != null) {
                            if (AssociateRole.ReportRoleManagesTheJSON[0] == null) {
                                var json =
                                {
        ...
        UserTableRowClass: function (row, rowIndex) {
            if ((AssociateRole.selectedIndex) === rowIndex) {

                return { "background-color": " #c2d6ea !important;" }
            }

        },
        //文件夹树点击节点
        //角色选中row 复选框选中
        RolCheckedSelect: function (row, column, event) {
            debugger
            IsHasPermissionsData = true;
        ...
        //得到返回所有的选中数据
        RolhandleSelectionChange: function (rows) {
            AssociateRole.ModifyRoleIdList = rows;
        },
       
       
        GetRolTableList: function () {

            AssociateRole.GetListRoles(1,'',1);
        },
        GetUserTableList: function () {
            AssociateRole.GetUsetDataTableList();
        },
    ...
    watch: {
        MeunPermissionsNameWhere: function (val, oldval) {

            AssociateRole.$refs.Menutree.filter(val);
        },
        SearchReportName: function (val, oldval) {

            AssociateRole.$refs.ReportTree.filter(val);
        },
        SearchUploadName: function (val, oldval) {

...
var ModelVM = new Vue({
  ...
  methods: {
    InitAttributesTable: function InitAttributesTable(id) {
      var that = this;
      ...
      $.post("/MasterDataManage/AttributesGet", {
        ...
        that.loading = false; //console.log(RES);
    ...
    ChangeValue: function ChangeValue(row, column, cellValue, index) {
      if (cellValue == "1") {
        return "是";
      } else {
        return "否";
    ...
    handleDelete: function handleDelete(index, row) {
      ...
      that.$confirm('此操作将删除该属性, 是否继续?', '提示', {
        ...
        $.post("/MasterDataManage/AttributeDel", {
          Id: id
        }, function (RES) {
          //console.log(RES);
          if (RES.success) {
    ...
    handleEdit: function handleEdit(index, row) {
      ...
      this.$nextTick(function () {
        _this.dialogTitle = "编辑属性";
        _this.AttributesRuleForm.Id = row.id;
        _this.AttributesRuleForm.LinkEntityID = row.linkEntityID == 0 ? "" : row.linkEntityID;

        if (_this.AttributesRuleForm.LinkEntityID) {
          _this.EntitySelectOptions2 = _this.AllEntityOptions;
        }

        _this.AttributesRuleForm.LinkModelID = row.linkModelID == 0 ? "" : row.linkModelID;
        _this.AttributesRuleForm.Name = row.name;
        _this.AttributesRuleForm.Remark = row.remark;
        _this.AttributesRuleForm.Type = row.type;
        _this.AttributesRuleForm.StartTrace = row.startTrace == "1" ? true : false;
        _this.AttributesRuleForm.IsDisplay = row.isDisplay == 1 ? true : false;
        _this.AttributesRuleForm.TypeLength = row.typeLength;
        _this.AttributesRuleForm.DisplayName = row.displayName;
        _this.AttributesRuleForm.Creater = row.Creater;
        _this.AttributesRuleForm.CreateTime = row.createTime;
        _this.AttributesRuleForm.Updater = row.name;
        _this.AttributesRuleForm.UpdateTime = row.updateTime;
        _this.AttributesRuleForm.DataType = row.dataType;
      });
    },
    ...
    AddOrUpdate: function AddOrUpdate() {
      ...
      ModelVM.$refs.AttributesRuleForm.validate(function (valid) {
        if (valid) {
          var that = _this2;
          var url = "";
          ...
          $.post(url, {
            attribute: JSON.stringify(that.AttributesRuleForm)
          }, function (RES) {
            //console.log(RES);
            if (RES.success) {
              that.$message({
    ...
    ModelHasChanged: function ModelHasChanged() {
      ...
      this.EntitySelectOptions = this.AllEntityOptions.filter(function (o) {
        return o.modelId == _this3.ModelID;
      });
    },
    ...
    ModelHasChanged2: function ModelHasChanged2() {
      ...
      this.EntitySelectOptions2 = this.AllEntityOptions.filter(function (o) {
        return o.modelId == _this4.AttributesRuleForm.LinkModelID;
      });
    },

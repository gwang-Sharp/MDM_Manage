  ...
  methods: {
    ...
    InitModelSelectOptions: function InitModelSelectOptions() {
      ...
      $.post("/system/Attributes_ModelsGet", function (RES) {
        //console.log(RES);
        if (RES.success) {
          that.ModelOptions = RES.data; //console.log(that.ModelOptions);
        } else {
          that.ModelOptions = [];
        }
      });
    ...
    handleEdit: function handleEdit(row) {
      ...
      this.$nextTick(function () {
        _this2.dialogTitle = "编辑实体";
        _this2.EntityRuleForm.Id = row.id;
        _this2.EntityRuleForm.Name = row.name;
        _this2.EntityRuleForm.Remark = row.remark;
        _this2.EntityRuleForm.StageTable = row.stageTable;
        _this2.EntityRuleForm.StartTime = row.createTime;
        _this2.EntityRuleForm.BeganIn = row.beganIn;
        _this2.EntityRuleForm.Model = row.modelId;

        if (row.autoCreateCode == "是") {
          _this2.EntityRuleForm.isCreate = true;
        } else {
          _this2.EntityRuleForm.isCreate = false;
        }
      });
    },
    //  面板关闭

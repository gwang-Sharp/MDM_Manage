...
var RoleManage = new Vue({
    ...
    data: {
        ...
        Title: '添加角色',//弹框标题
        AddRoledialogVisible: false,//添加角色弹框是否显示
        //添加用户角色form表单
        RoleForm: {
            RoleName: '',
            RoleDesc: ''
        ...
        //验证Roleform表单规则
        rules: {
            RoleName: [{ required: true, message: "请输入角色名称", trigger: 'blur' }], //动态给表格高度
    ...
    mounted: function () {
        this.$nextTick(() => {
            ...
            //this.UploadUserRolePermissions();
        });
    },
    methods: {
        //进入页面渲染表格
        GetRoleTableList: function() {
        ...
        OpenRoleEditDialog: function(index, row) {
            RoleManage.AddRoledialogVisible = true;
            RoleManage.$nextTick(() => {
            ...
            RoleManage.$confirm("是否删除","角色删除", {
                ...
                callback: function callback(action, instance) {
                    if (action === 'confirm') {
                        ...
                        $.post('/System/DeleteRole', { id: row.id }, function (res) {
                            loading.close();
                            if (res.success == true) {
                                RoleManage.GetRoleTableList();
        ...
        //关闭添加用户对话框或者修改用户对话框
        CloseRoledialogVisible: function(RoleForm) {
            RoleManage.$nextTick(() => {
                RoleManage.AddRoledialogVisible = false;
        ...
        SubmitRoleForm: function () { //提交角色表单
            if (RoleManage.EditOrAdd == 'Add') {
                RoleManage.$refs.RoleForm.validate((valid) => {
                    if (valid) {
                        var loading = RoleManage.$loading({
                            lock: true,
                            text: '加载中……',
                        ...
                        $.post('/System/InserRole', this.RoleForm, function (res) {
                            loading.close();
                            if (res.success == true) {
                                debugger
                                RoleManage.$nextTick(() => {
                ...
                RoleManage.$refs.RoleForm.validate((valid) => {
                    if (valid) {
                        ...
                        $.post('/System/UpdateRole', this.RoleForm, function (res) {
                            loading.close();
                            if (res.success == true) {
                                RoleManage.$nextTick(() => {
                                    RoleManage.$message.success(res.message);
